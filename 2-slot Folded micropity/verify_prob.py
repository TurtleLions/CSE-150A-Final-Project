...
def verify_probabilities():
    ...
    for case_name in ['Case 1', 'Case 2', 'Case 3', 'Case 4']:
        ...
        rate_10_theo, rate_11_theo = folded_rates[case_name]
        ...
        results.append({
            ...
            'R10 SSR (Folded)': f"{rate_10_theo:.4f}",
            'R10 SSR (Obs)': f"{rate_10_obs:.4f}",
            'R11 SSR (Folded)': f"{rate_11_theo:.4f}",
            'R11 SSR (Obs)': f"{rate_11_obs:.4f}"
        })

    res_df = pd.DataFrame(results)
    
    print("Comparison of Folded Theoretical vs Observed Distributions & Rates:")
